...
function solution(someTime, leavingTime) {
  const currentTime = new Date(someTime);
  const meetingTime = new Date(leavingTime);
  const timeDiff = Math.abs(meetingTime.getTime() - currentTime.getTime());

  return new Date(currentTime.getTime() - timeDiff)
    .toISOString()
    .replace(/T/, " ")
    .slice(0, 16);
}
